...
(function($){
	_PdmMediabrowserUtility = {
		...
		openDialog: function(url, id, width, height, title, options) {
			var windowId = id || this.windowId,
			...
			if (this.modal) {
				...
				this.modal = $(content).modal($.extend({
					// title:  title || 'SM Page Builder Add Widget...',
					// modalClass: 'magento col-sm-12',
					// type: 'slide',
					// buttons: []
				}, options));
			}
    ...
    //
	// 		if (!targetEl.length) {
	// 			this.options.onInsertCallback = 'CmrSl.addLayerImage()';
	// 			//_PdmMediabrowserUtility.closeDialog();
	...
	// 			}
	// 			_PdmMediabrowserUtility.closeDialog();
	// 			if (this.options.onInsertCallback) {
	// 				var _index = null;
	// 				if(CmrSl.selectedLayer)
	// 				{
	// 					_index = CmrSl.selectedLayer.params.serial;
	// 				}
	// 				CmrSl.addLayerImage(data, _index);
	// 			}
    //
	// 			targetEl.focus();
	// 		}, this));
